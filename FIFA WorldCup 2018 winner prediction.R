...
setwd("C:/Users/PC/Desktop/FIFA WorldCup 2018 winner prediction/new")

# loading dataset
world_cup_dataset <- read.csv("WorldCup2018Dataset.csv", stringsAsFactors = FALSE, header = TRUE)
...
#    for a zero sum game such as football.

# Data Cleaning

# Loading all the playing nations
...
# Changing country name "IR Iran" to "Iran"
team_rankings[team_rankings$country_full == "IR Iran", "country_full"] <- "Iran"
# Changing country name "Korea Republic" to "South Korea"
...
ggplot(elo_10, aes(x=country, y=elo_rating)) + geom_bar(stat = "identity", fill = "orange") + labs(x="Team", y="Elo Rating", title="Top 10 El Rated Teams in the World")  +
  theme(plot.title = element_text(hjust = 0.5))
